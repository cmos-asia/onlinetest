...
public class TestRecordController extends BaseController {
	...
	public String list(HttpServletRequest request, Model model,
			TestRecord testRecord) throws Exception {
	...
	public @ResponseBody ReturnDatas listjson(HttpServletRequest request,
			Model model, TestRecord testRecord) throws Exception {
		ReturnDatas returnObject = ReturnDatas.getSuccessReturnDatas();
		// ==构造分页请求
		Page page = newPage(request);
		// ==执行分页查询
		List<TestRecord> datas = testRecordService.findListDataByFinder(null,
				page, TestRecord.class, testRecord);
		returnObject.setQueryBean(testRecord);
		if (CollectionUtils.isNotEmpty(datas)) {
			for (TestRecord tr : datas) {
				Student student = studentService.findStudentById(tr
						.getStudent_id());
				if (StringUtils.isNotBlank(student.getName())) {
					tr.setStudent_name(student.getName());
				} else {
					tr.setStudent_name(student.getPhone());
				}
			}
		}
	...
	public void listexport(HttpServletRequest request,
			HttpServletResponse response, Model model, TestRecord testRecord)
			throws Exception {
		// ==构造分页请求
		Page page = newPage(request);

		File file = testRecordService.findDataExportExcel(null, listurl, page,
				TestRecord.class, testRecord);
	...
	public @ResponseBody ReturnDatas saveorupdate(Model model,
			TestRecord testRecord, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ReturnDatas returnObject = ReturnDatas.getSuccessReturnDatas();
